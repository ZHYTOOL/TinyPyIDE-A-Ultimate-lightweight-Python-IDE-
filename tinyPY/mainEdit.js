...
function Do(task){
    ...
    //     alert(response)
    // })
    if (task.includes('Complete>')) {
        const completion = task.split('>')[1];          // 获取要插入的代码
        const cursorPos = infoGet.selectionStart;       // 当前光标位置
        const text = infoGet.value;
        // 在光标处插入补全文本
        infoGet.value = text.slice(0, cursorPos) + completion + text.slice(cursorPos);
        // 移动光标到插入内容之后
        infoGet.selectionStart = infoGet.selectionEnd = cursorPos + completion.length;
        fitInfo();                                       // 更新界面
        // 关闭补全列表
        ...
        setTimeout(() => {
            ...
            if(before!='\n'&&before!=''){
                window.pywebview.api.complete(infoGet.value).then((response) => {
                    //let html=`<div style="position:absolute;top:${top}px;left:${left}px;text-align:left;flex:wrap;" id="chooses">`
                    let html=''
                    for(let i in response){
                        if(i==choose) {
                            html += `<button class="popup-option" style="color:brown; display: block;" onclick="Do('Complete>${response[i][1]}')">${response[i][0]} => ${response[i][0]}</button>`
                        }
                        else{
                            html += `<button class="popup-option" style="display: block;" onclick="Do('Complete>${response[i][1]}')">${response[i][1]} => ${response[i][0]}</button>`
                        }
                    }
                    //html+="</div>"
                    document.getElementById('choice').innerHTML=html;
                    document.getElementById('choice').style=`position:absolute;top:${top}px;left:${left}px;text-align:left;flex:wrap; `
                })
            }
            else{
                document.getElementById('choice').innerHTML=''
...
infoGet.addEventListener('keydown', function (event) {
    ...
    if (event.key === 'Tab') {
        event.preventDefault();
        if (hasChoices) {
            // console.log('tab cpl',choose >= 0 , choose , choiceDiv.children.length)
            // 有补全列表：插入当前高亮项
